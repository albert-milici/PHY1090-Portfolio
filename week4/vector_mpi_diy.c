...
int main(int argc, char **argv) 
{
	...
	if (0 == my_rank)
	{
		// root creates and initialises the full vector
		int *full_vector = (int *) malloc(num_arg * sizeof(int));
		initialise_vector(full_vector, num_arg);

		// copies root's own chunk into its local buffer
		for (int i = 0; i < chunk; i++)
		{
			local_chunk[i] = full_vector[i];
		}

		// sends each client's chunk using pointer arithmetic
		for (int dest = 1; dest < uni_size; dest++)
		{
			MPI_Send(full_vector + dest * chunk, chunk, MPI_INT, dest, tag, MPI_COMM_WORLD);
		} // end for (int dest = 1; dest < uni_size; dest++)

		// frees the full vector
		free(full_vector);
	} // end if (0 == my_rank)
	else // i.e. (0 != my_rank)
	{
		MPI_Status status;
		// receives this process's chunk from root
		MPI_Recv(local_chunk, chunk, MPI_INT, 0, tag, MPI_COMM_WORLD, &status);
	...
	if (0 == my_rank)
	{
		printf("DIY distribution time: %.9lf s\n", dist_time);
	}
