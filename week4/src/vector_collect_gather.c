...
int main(int argc, char **argv) 
{
	...
	// root allocates an array to gather all partial sums
	int *all_sums = NULL;
	if (0 == my_rank)
	{
		all_sums = (int *) malloc(uni_size * sizeof(int));
	}

	// creates and initialises the timing variables for collection
	double collect_time = 0.0;

	// times the collection step
	timespec_get(&start_time, TIME_UTC);
	// gathers all partial sums into the array on root
	...
	// root sums the gathered partial sums and prints
	if (0 == my_rank)
	{
		int total_sum = sum_vector(all_sums, uni_size);
		printf("Sum: %d\n", total_sum);
		printf("Gather collection time: %.9lf s\n", collect_time);
		free(all_sums);
	}
